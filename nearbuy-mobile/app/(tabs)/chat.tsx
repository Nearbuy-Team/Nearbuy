...
export default function Chat() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      for (const message of messages) {
        ...
        if (!latest.has(key)) latest.set(key, { listingId: message.listingId, otherUserId, last: message });
      }
      const next = Array.from(latest.values());
      const profiles = await Promise.all(
        next.map((thread) => usersApi.publicProfile(token, thread.otherUserId).catch(() => undefined))
      );
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: c.canvas }}>
      ...
      <Text style={{ fontFamily: FONTS.extrabold, fontSize: 22, color: c.ink, letterSpacing: -0.5, marginTop: 8, paddingHorizontal: 20 }}>Messages</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 16, paddingHorizontal: 20, paddingBottom: 110 }}>
        ...
        {!loading && !error && threads.length === 0 ? (
          <Text style={{ fontFamily: FONTS.medium, color: c.secondary }}>No conversations yet. Open a listing to message its seller.</Text>
        ) : null}
        <View style={{ gap: 12 }}>
          {threads.map((thread) => {
            ...
            return (
              <Pressable
                ...
                style={({ pressed }) => ({ flexDirection: 'row', alignItems: 'center', gap: 13, backgroundColor: c.surface, borderRadius: 16, paddingVertical: 13, paddingHorizontal: 14, transform: [{ scale: pressed ? 0.985 : 1 }], ...SHADOWS.row })}>
                <View style={{ width: 46, height: 46, borderRadius: 14, backgroundColor: theme.tagBg, borderWidth: 1, borderColor: theme.tagBorder, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ fontFamily: FONTS.extrabold, fontSize: 16, color: ON_TINT }}>{initial || 'N'}</Text>
                </View>
                <View style={{ flex: 1, minWidth: 0 }}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                    <Text numberOfLines={1} style={{ flex: 1, fontFamily: FONTS.extrabold, fontSize: 14.5, color: c.ink }}>{name}</Text>
                    <Text style={{ fontFamily: FONTS.semibold, fontSize: 11, color: c.muted }}>{new Date(thread.last.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                  </View>
                  <Text numberOfLines={1} style={{ fontFamily: FONTS.medium, fontSize: 12.5, color: c.secondary, marginTop: 3 }}>{thread.last.content}</Text>
                </View>
              </Pressable>
            );
          })}
