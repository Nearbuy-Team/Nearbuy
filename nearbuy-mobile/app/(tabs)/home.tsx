...
export default function Home() {
  ...
  const cards = useMemo(() => {
    ...
    return all.filter((listing) =>
      [listing.title, listing.description || '', listing.type].join(' ').toLowerCase().includes(q)
    );
  }, [listings, mode, q]);
    ...
    <View style={{ flex: 1, backgroundColor: c.canvas }}>
      ...
      <KeyboardSafeView>
        ...
        <ScrollView
          ...
          {!!error && (
            <Text style={{ fontFamily: FONTS.semibold, color: c.danger, marginBottom: 12 }}>
              {error}
            </Text>
          )}
          ...
          {isLoading && listings.length === 0 ? (
            ...
            <Animated.View
              ...
              style={{ alignItems: 'center', paddingTop: 48, gap: 14 }}>
              <View
                style={{
                  alignItems: 'center',
